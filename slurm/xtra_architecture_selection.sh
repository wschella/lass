...
for model in "${models[@]}"; do
  ...
  srun --output T${time}xtra_archsel_${model}.out -c 2 --mem 40G --exclusive --nodes 1 --ntasks 1 python src/lass/experiments/xtra_architecture_selection.py --model $model --time $time "$@" &
done
wait
